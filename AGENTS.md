...
6. Open `index.html`.
   ...
   one-line description. The sheet number chip and the Activity/Project
   chip are generated by CSS, so nothing extra needs to be typed.

   Optional: to make the new card's image "fly" into its detail page hero,
   add one `view-transition-name` line for the new image filename in both
   `index.css` and `project.css`, next to the existing six rules.
